...
const firstLetters = (arr) => {
  let newArr = [];
  arr.forEach(string => newArr.push(string.substring(0, 1)));
...
const standardizePhoneNumbers = (arr) => {
  let newArr = [];
  arr.map(string => {
    let first = string.substring(1, 4);
    let second = string.substring(6, 9);
    let third = string.substring(10, string.length);
...
const onlyOddChars = (str) => {
  let odds = [1, 3, 5, 7, 9];
  let thing = odds.map(num => str.charAt(num));
  return thing.join('');
};
