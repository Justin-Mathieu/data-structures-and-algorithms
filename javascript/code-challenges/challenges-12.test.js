...
const validateEmail = (email) => {
  let pattern = /^[a-zA-z]+[.]{0,1}[a-zA-z]+@+[a-zA-Z]+[.]+[a-zA-Z]{3}\b/gm;
...
const validatePhoneNumber = (phoneNumber) => {
  let pattern = /^(1\s?)?(\d{3}|\(\d{3}\))[\s\-]?\d{3}[\s\-]?\d{4}$/im;
