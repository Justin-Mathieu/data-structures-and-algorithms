...
function transformToLis(obj) {
  let arr = [];
  Object.entries(obj).forEach(person => arr.push(`<li>${person[0]}: ${person[1]}</li>`));
  return arr;
}
...
const totalSum = (input) => {
  let thing = [];
  input.forEach(outer => thing.push(outer.reduce((acc, curr) => acc += curr)));
  return thing.reduce((acc, curr) => acc += curr);
};
...
let findMaleAndFemale = (data) => {
  const arr = [];
  data.forEach(thing => {
    if (thing.gender === 'female') {
      arr.push(thing.name);
    }
    else if (thing.gender === 'male')
      arr.push(thing.name);
  });
  return arr.toString().replace(/,/g, ' and ');
};
...
let findShortest = (data) => {
  ...
  const num = data.map(obj => obj.height).reduce((a, b) => a < b ? b : a);
  let arr = data.filter(thing => thing.height === num);
